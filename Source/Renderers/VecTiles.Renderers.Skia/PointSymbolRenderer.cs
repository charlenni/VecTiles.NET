...
public class PointSymbolRenderer : ISymbolRenderer
{
    public static bool CheckForSpace(SKCanvas canvas, EvaluationContext context, ISymbol sym, Quadtree<ISymbol> tree, Func<double, double, (double, double)> worldToScreenConverter, bool showUnvalidBorders = false)
    {
        ...
        bool spaceForIconAvailable = IconPointSymbolRenderer.CheckForSpace(canvas, context, symbol.IconSymbol, tree, worldToScreenConverter, showUnvalidBorders);
        bool spaceForTextAvailable = TextPointSymbolRenderer.CheckForSpace(canvas, context, symbol.TextSymbol, tree, worldToScreenConverter, showUnvalidBorders);
    ...
    public static void Draw(SKCanvas canvas, EvaluationContext context, ISymbol sym, ref Quadtree<ISymbol> tree, Func<double, double, (double, double)> worldToScreenConverter, bool showValidBorders = false)
    {
        ...
        {
            TextPointSymbolRenderer.Draw(canvas, context, symbol.TextSymbol, ref tree, worldToScreenConverter, showValidBorders);
        }
    }
}