...
public static class OMTFillPaint
{
    public static Paint CreatePaint(ILayerStyle style, Func<string, Sprite> spriteFactory)
    {
        if (style is not OMTLayerStyle omtStyle)
        {
            throw new ArgumentException($"Style {style.GetType()} isn't a OMTLayerStyle");
        }

        var layout = omtStyle.Layout;
        var paint = omtStyle.Paint;
        ...
        {
            brush.SetFixColor(omtStyle.Paint.FillColor.Evaluate(EvaluationContext.Empty));
        }
        else
        {
            brush.SetVariableColor((context) => omtStyle.Paint.FillColor.Evaluate(context));
        }
        ...
        {
            brush.SetFixOpacity(omtStyle.Paint.FillOpacity.Evaluate(EvaluationContext.Empty));
        }
        else
        {
            brush.SetVariableOpacity((context) => omtStyle.Paint.FillOpacity.Evaluate(context));
        }
        ...
        {
            brush.SetFixAntialias(omtStyle.Paint.FillAntialias.Evaluate(EvaluationContext.Empty));
        }
        else
        {
            brush.SetVariableAntialias((context) => omtStyle.Paint.FillAntialias.Evaluate(context));
        }
        ...
        {
            brush.SetFixOutlineColor(omtStyle.Paint.FillOutlineColor.Evaluate(EvaluationContext.Empty));
        }
        else
        {
            brush.SetVariableOutlineColor((context) => omtStyle.Paint.FillOutlineColor!.Evaluate(context));
        }
            ...
            if (paint.FillPattern.IsConstant)
            {
                ...
                if (!string.IsNullOrEmpty(sprite))
                {
                    if (sprite.Contains("{"))
                    {
                        ...
                        {
                            var name = omtStyle.Paint.FillPattern.Evaluate(context).ReplaceFields(context.Attributes);
